...
def collect_referenced_types(schema: Dict[str, Any], own_name: str) -> Set[str]:
    refs: Set[str] = set()
    ...
    def collect(value: Any) -> None:
        ...
        if not isinstance(value, dict):
            return
        if "$ref" in value:
            refs.add(resolve_ref(value["$ref"]))
            return
        if "$id" in value:
            refs.add(value["$id"])
        for v in value.values():
...
def generate_model_file(
    ...
    refs = collect_referenced_types(schema, class_name)
    lines = [
    ...
    if refs:
        ...
        for ref in sorted(refs):
            ...
            lines.append(f"from .{module_name} import {ref}")

    lines.append("")
    lines.append("")
    lines.append(generate_type_definition(class_name, schema, schemas_by_id))
    lines.append("")
...
def generate_api_types() -> None:
    ...
    models_dir = "src/judgeval/internal/api/models"
    ...
    for schema_id in sorted(schemas_by_id.keys()):
        ...
        model_code = generate_model_file(schema_id, schema, schemas_by_id)
        file_name = to_snake_case(schema_id)
        file_path = os.path.join(models_dir, f"{file_name}.py")
        with open(file_path, "w") as f:
            f.write(model_code)
        schema_names.append(schema_id)
        print(f"Generated model: {file_path}", file=sys.stderr)
    ...
    for name in schema_names:
        ...
        init_lines.append(f"from .{module_name} import {name} as {name}")
    init_lines.append("")

    init_path = os.path.join(models_dir, "__init__.py")
    with open(init_path, "w") as f:
        f.write("\n".join(init_lines))
    print(f"Generated models __init__: {init_path}", file=sys.stderr)
    print(f"Generated {len(schema_names)} model files", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    api_dir = "src/judgeval/internal/api"
    models_dir = os.path.join(api_dir, "models")
    os.makedirs(api_dir, exist_ok=True)

    old_types = os.path.join(api_dir, "api_types.py")
    ...
    client_path = os.path.join(api_dir, "api_client.py")
