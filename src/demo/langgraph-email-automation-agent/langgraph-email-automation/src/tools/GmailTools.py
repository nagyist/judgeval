...
# from ..agents import judgment # Example if initialized in agents.py (adjust path)
...
class GmailToolsClass:
    ...
    @judgment.observe()
    def create_draft(self, recipient, subject, body_text):
        """
        Creates a new email draft (not a reply).

        @param recipient: Email address of the recipient.
        @param subject: Subject line of the email.
        @param body_text: The plain text content of the email body.
        @return: The created draft object from the Gmail API, or None if an error occurred.
        """
        try:
            # Create the basic email message (MIMEMultipart for potential HTML later)
            message = MIMEMultipart("alternative")
            message["to"] = recipient
            message["subject"] = subject
            # Optionally set 'from' if needed, but Gmail usually handles this
            # message["from"] = os.environ['MY_EMAIL'] 

            # --- Improved HTML Formatting --- 
            # Replace double newlines with paragraph breaks for better structure
            ...
            # Attach parts
            # text_part = MIMEText(body_text, "plain") # Plain text fallback (optional but good practice)
            ...
            # Prepare the request body for the drafts.create endpoint
            create_draft_request_body = {
                'message': {
                    'raw': encoded_message
                }
                # Note: No threadId is included for a new draft
            }
