...
from contextlib import contextmanager, asynccontextmanager, AbstractAsyncContextManager, AbstractContextManager
...
# Add Iterator and AsyncIterator
from collections.abc import Iterator, AsyncIterator

# Define context variables for tracking the current trace and the current span within a trace
current_trace_var = contextvars.ContextVar[Optional['TraceClient']](
    'current_trace', default=None)
current_span_var = contextvars.ContextVar(
    'current_span', default=None)  # ContextVar for the active span name

# Define type aliases for better code readability and maintainability
ApiClient: TypeAlias = Union[OpenAI, Together, Anthropic, AsyncOpenAI, AsyncAnthropic,
                             AsyncTogether, genai.Client, genai.client.AsyncClient]  # Supported API clients
...
class TraceManagerClient:
    ...
    def save_trace(self, trace_data: dict, offline_mode: bool = False):
        ...
        serialized_trace_data = json.dumps(
            trace_data, default=fallback_encoder)
...
class TraceClient:
    ...
    def __init__(
        ...
        # NEW: To track depth of active spans
    ...
    def async_evaluate(
        ...
        eval_run = EvaluationRun(
            ...
            # Keep original eval name format using context var if available
            f"{span_id_to_use}-"
        ...
        # Pass start_time to record_evaluation
    ...
    def record_output(self, output: Any):
        ...
        if current_span_id:
            span = self.span_id_to_span[current_span_id]
            span.output = "<pending>" if inspect.iscoroutine(
                output) else output

            if inspect.iscoroutine(output):
                asyncio.create_task(
                    self._update_coroutine(span, output, "output"))
    ...
    def save(self, overwrite: bool = False) -> Tuple[str, dict]:
        ...
        # Safe to save, proceed normally
        return self._perform_actual_save(overwrite)

    def _perform_actual_save(self, overwrite: bool = False) -> Tuple[str, dict]:
...
class _DeepTracer:
    ...
    def _should_trace(self, frame):
        ...
        if (
            ...
            # dunders
            or func_name.startswith("__") and func_name != "__call__"
    ...
    def _cooperative_sys_trace(self, frame: types.FrameType, event: str, arg: Any):
        ...
        our_result = self._trace(
            frame, event, arg, self._cooperative_sys_trace)
    ...
    def _cooperative_threading_trace(self, frame: types.FrameType, event: str, arg: Any):
        ...
        if self._original_threading_trace:
            try:
                original_result = self._original_threading_trace(
                    frame, event, arg)
            except Exception:
                # If the original tracer fails, continue with our tracing
                pass
        ...
        # Then do our own tracing
        our_result = self._trace(
            frame, event, arg, self._cooperative_threading_trace)
    ...
    def _trace(self, frame: types.FrameType, event: str, arg: Any, continuation_func: Callable):
        ...
        if 'self' in frame.f_locals:
            ...
            class_identifiers = getattr(
                Tracer._instance, 'class_identifiers', {})
        ...
        if event == "call":
            ...
            current_trace.span_id_to_span[span_data["span_id"]
                                          ].duration = duration
...
class Tracer:
    ...
    def __init__(
        ...
        enable_monitoring: bool = os.getenv(
            "JUDGMENT_MONITORING", "true").lower() == "true",
        enable_evaluations: bool = os.getenv(
            "JUDGMENT_EVALUATIONS", "true").lower() == "true",
        # S3 configuration
        use_s3: bool = False,
        ...
        if not hasattr(self, 'initialized'):
            if not api_key:
                raise ValueError(
                    "Tracer must be configured with a Judgment API key")
            ...
            if not organization_id:
                raise ValueError(
                    "Tracer must be configured with an Organization ID")
            if use_s3 and not s3_bucket_name:
                raise ValueError(
                    "S3 bucket name must be provided when use_s3 is True")

            self.api_key: str = api_key
            self.project_name: str = project_name or str(uuid.uuid4())
            self.organization_id: str = organization_id
            self._current_trace: Optional[str] = None
            # Add active trace client attribute
            self._active_trace_client: Optional[TraceClient] = None
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                if not current_trace:
                    ...
                    try:
                        ...
                        # MODIFIED: Use span_name directly
                        ...
                        trace_id, trace = current_trace.save(
                            overwrite=overwrite)
    ...
    def async_evaluate(self, *args, **kwargs):
        ...
        if current_trace:
            ...
            if passed_trace_id:
                # Re-add if needed by TraceClient.async_evaluate
...
def wrap(client: Any) -> Any:
    ...
    """
    span_name, original_create, original_responses_create, original_stream = _get_client_config(
        client)
    ...
    def traced_response_create_sync(*args, **kwargs):
        ...
        with current_trace.span(span_name, span_type="llm") as span:
            ...
            try:
                response_or_iterator = original_responses_create(
                    *args, **kwargs)
...
async def _async_stream_wrapper(
    ...
    try:
        ...
        if final_usage_data:
            ...
            usage_info = _extract_usage_from_final_chunk(
                client, last_content_chunk)
...
def get_instance_prefixed_name(instance, class_name, class_identifiers):
    ...
    if class_name in class_identifiers:
        ...
        if hasattr(instance, attr):
            ...
            raise Exception(
                f"Attribute {class_identifiers[class_name]} does not exist for {class_name}. Check your identify() decorator.")
    return None
