                ...
                if model_name:
                    try:
                        ...
                        if "usage" not in output:
                            ...
                            print(f"[WARN TraceClient.save] Output 'usage' for span {span.get('span_id')} was not a dict ({type(output['usage'])}). Resetting before adding costs.")
                        ...
                        print(f"Error calculating cost for model '{model_name}' (span: {span.get('span_id')}): {str(e)}")
                        pass
                else:
                     print(f"[WARN TraceClient.save] Could not determine model name for cost calculation (span: {span.get('span_id')}). Inputs: {span_inputs}")
...
class _TracedAsyncStreamManagerWrapper(AbstractAsyncContextManager):
    ...
    async def __aenter__(self):
        ...
        wrapped_iterator = self._stream_wrapper_func(raw_iterator, self._client, span)
