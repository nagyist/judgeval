...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server" and self.accelerator.is_main_process:
                ...
                print(f"DEBUG: vLLM initialized with max_model_len={self.max_prompt_length + self.max_completion_length} (prompt={self.max_prompt_length}, completion={self.max_completion_length})")

                self.env.set_vllm_model(self.llm)
    ...
    def _generate_and_score_completions(
        ...
        # tokens for prompts, completions and environment response - masks

        # Move tensors to the same device as prompt_ids
        completion_ids = completion_ids.to(prompt_ids.device)
        assistant_mask = assistant_mask.to(prompt_ids.device)
        completion_mask = completion_mask.to(prompt_ids.device)
        rewards = rewards.to(prompt_ids.device)

        assistant_lengths = assistant_mask.sum(1)  # Use assistant_mask for completion lengths
        logits_to_keep = completion_ids.size(1)


        prompt_completion_ids = torch.cat([prompt_ids, completion_ids], dim=1)
        attention_mask = torch.cat([prompt_mask, completion_mask], dim=1)  # Use completion_mask for attention
        ...
        agg_completion_lengths = self.accelerator.gather(assistant_lengths)
    ...
    def _compute_loss(self, model, inputs):
        # Compute the per-token log probabilities for the model
        prompt_ids, prompt_mask = inputs["prompt_ids"], inputs["prompt_mask"]
        completion_ids, completion_mask, assistant_mask = inputs["completion_ids"], inputs["completion_mask"], inputs["assistant_mask"]
        input_ids = torch.cat([prompt_ids, completion_ids], dim=1)
        attention_mask = torch.cat([prompt_mask, completion_mask], dim=1)
        logits_to_keep = completion_ids.size(1)  # we only need to compute the logits for the completion tokens
