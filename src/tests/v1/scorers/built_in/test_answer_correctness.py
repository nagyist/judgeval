...
def test_answer_correctness_with_strict_mode():
    scorer = AnswerCorrectnessScorer(strict_mode=True)
...
def test_answer_correctness_get_scorer_config():
    scorer = AnswerCorrectnessScorer(
        threshold=0.6, name="Test Scorer", strict_mode=True, model="gpt-4"
    )
