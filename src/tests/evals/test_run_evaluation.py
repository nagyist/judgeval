...
class TestRunEvaluation:
    ...
    def test_merge_results(self, mock_scoring_results):
        ...
        assert merged[0].scorers_data == api_results[0].scorers_data + local_results[0].scorers_data
    ...
    @patch('requests.post')
    def test_log_evaluation_results_success(self, mock_post, mock_scoring_results, mock_evaluation_run):
        mock_post.return_value.ok = True
    ...
    async def test_poll_evaluation_until_complete(self):
        with patch('requests.get') as mock_get:
            mock_get.side_effect = [
                ...
                Mock(ok=True, json=lambda: {"status": "completed", "results": [{"success": True}]})
            ]
            
            # Mock the results fetch endpoint
            with patch('requests.post') as mock_post, patch('asyncio.sleep') as mock_sleep:
                mock_post.return_value.ok = True
    ...
    def test_run_eval_sync(self, mock_evaluation_run):
        with patch('judgeval.run_evaluation.execute_api_eval') as mock_execute, \
             patch('builtins.input', return_value='y'):  # Mock input to return 'y'
            mock_execute.return_value = {
                "results": [{
                    "success": True,
                    "scorers_data": [{
