...
from judgeval.utils.wrappers.mutable_wrap_sync import mutable_wrap_sync
...
def test_basic_functionality():
    ...
    wrapped = mutable_wrap_sync(add)
    result = wrapped(2, 3)
    assert result == 5


def test_mutate_kwargs_hook_modifies_kwargs():
    ...
    def func(value: int = 0) -> int:
        call_log.append(("func", value))
        return value

    wrapped = mutable_wrap_sync(func, mutate_kwargs_hook=mutate_kwargs)
...
def test_mutate_args_hook_modifies_args():
    ...
    wrapped = mutable_wrap_sync(func, mutate_args_hook=mutate_args)
    result = wrapped(5, 3)
...
def test_mutate_kwargs_hook_failure_uses_identity():
    ...
    wrapped = mutable_wrap_sync(func, mutate_kwargs_hook=failing_mutate_kwargs)
    result = wrapped(value=5)
...
def test_mutate_args_hook_failure_uses_identity():
    ...
    wrapped = mutable_wrap_sync(func, mutate_args_hook=failing_mutate_args)
    result = wrapped(5, 3)
...
def test_mutate_both_args_and_kwargs():
    ...
    result = wrapped(5, 3, multiplier=2)
...
def test_mutate_kwargs_hook_with_context():
...
def test_mutate_hook_with_mutate_kwargs():
...
def test_pre_hook_called_before_kwargs_mutation():
...
def test_error_hook_called_when_function_fails():
    """Test that error_hook is called when the wrapped function raises."""
    error_captured = {}
    ...
    wrapped = mutable_wrap_sync(failing_func, error_hook=error_hook)
...
def test_finally_hook_called_on_success():
    ...
    wrapped = mutable_wrap_sync(func, finally_hook=finally_hook)
    result = wrapped()
...
def test_finally_hook_called_on_failure():
    ...
    wrapped = mutable_wrap_sync(failing_func, finally_hook=finally_hook)
...
def test_kwargs_mutation_preserves_other_parameters():
    ...
    wrapped = mutable_wrap_sync(func, mutate_kwargs_hook=mutate_kwargs)
    result = wrapped(a=1, b=2, c=3)

    assert result == 1 + 12 + 3  # Only b was modified


def test_empty_kwargs_mutation():
...
def test_mutate_hook_failure_returns_original_result():
    ...
    wrapped = mutable_wrap_sync(func, mutate_hook=mutate_result)
    result = wrapped()
