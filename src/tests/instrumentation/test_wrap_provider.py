...
from judgeval.instrumentation.llm.config import _detect_provider, wrap_provider
...
class TestDetectProvider:
    def test_openai_sync_client(self):
        ...
        with (
            patch("judgeval.instrumentation.llm.config.HAS_OPENAI", True),
            patch("judgeval.instrumentation.llm.config.HAS_ANTHROPIC", False),
            patch("judgeval.instrumentation.llm.config.HAS_TOGETHER", False),
            patch("judgeval.instrumentation.llm.config.HAS_GOOGLE_GENAI", False),
        ):
            from openai import AsyncOpenAI

            with patch(
                "judgeval.instrumentation.llm.config._detect_provider",
