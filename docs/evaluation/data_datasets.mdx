...
We understand that our users have their data coming from a variety of sources and that the naming conventions may not perfectly align with our [Example fields](/evaluation/data_examples).
We give users the flexibility to plug and use their data with minimal file editing: simply provide a dictionary mapping of your organization's input fields (keys) to Judgment's fields (values)!

Additionally, we support custom primary and secondary delimiters to fit your data's needs and to distinguish fields with List and Collection types. For more information, see our API Reference (TODO)
...
# define a mapping of your custom headers to judgment's headers!
header_mapping = {
    # custom key: corresponding judgment header
    "my_input_field": "input",
    "our_llm_output": "actual_output",
    ...
}

new_dataset.add_from_csv("/path/to/your/csv/file.csv", header_mapping=header_mapping)
```
