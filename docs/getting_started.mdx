...
Ensure that you have Python 3.11 or later installed. In your command line, type the following to install Judgment:
...
Copy the following code into a `sample_eval.py` file, and run it on the command line using `python sample_eval.py`:
...
Below is some sample code to run our first trace. For this example to run, you will need an OpenAI API key and set it as an environment variable with:
...
Congratulations! You've just created your first trace. If you follow the link generated for you, it should look like this:
...
To run an online evaluation, we can import our scorer and modify our existing trace:
