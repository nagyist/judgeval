...
- **Emphasizes on top results**: WCP places a strong emphasis on the relevance of top-ranked results. This emphasis is important because LLMs tend to give more attention to earlier nodes in the `retrieval_context`. 
Therefore, improper rankings can induce hallucinations in the `actual_output`.
...
<Note>
Our contextual precision scorer is based on Stanford NLP's [ARES](https://arxiv.org/abs/2311.09476) paper (Saad-Falcon et. al., 2024).
</Note>
...
<Tabs items={['Python', 'TypeScript']}>
  ...
  <Tab value="TypeScript">
    ```ts
    import { JudgmentClient, ExampleBuilder, ContextualPrecisionScorer, logger } from 'judgeval';

    async function runContextualPrecision() {
        ...
        // supply your own threshold
        const scorer = new ContextualPrecisionScorer(0.8);

        const results = await client.evaluate({
            examples: [example],
            scorers: [scorer],
            model: "gpt-4o",
            projectName: "ctx-precision-ts-proj",
            evalName: "ctx-precision-ts-eval"
        });
        logger.print(results); // Log results to console
    }

    runContextualPrecision();
    ```
  </Tab>
</Tabs>

<Note>
The `ContextualPrecision` scorer uses an LLM judge, so you'll receive a reason for the score in the `reason` field of the results. 
